...
function checkApiConfiguration(): boolean {
  ...
  if (
    ...
    console.warn(
      "Google Sheets Spreadsheet ID not configured. Using demo mode.",
    );
    return false;
  }
  return true;
  ...
  return new Promise((resolve, reject) => {
    ...
    tokenClient.callback = async (response) => {
      ...
      if (response.access_token) {
        ...
        const existingEmail = existing
          ? (() => {
              try {
                return JSON.parse(existing).email;
              } catch {
                return null;
              }
            })()
...
export async function getSheetTabs(): Promise<string[]> {
  ...
  return withAutoRefresh(async () => {
    try {
      ...
      return (
        response.result.sheets?.map((sheet: any) => sheet.properties.title) ||
        []
      );
    } catch (error) {
...
export async function addTransaction(
  monthName: string,
  rowData: Omit<SheetRow, "id" | "createdAt" | "updatedAt">,
): Promise<SheetRow> {
  if (!isApiConfigured || !isSignedIn()) {
    throw new Error("Not authenticated");
  }
