...
describe('google-drive', () => {
  ...
  it('extractFileIdFromUrl supports common drive url patterns', () => {
    expect(
      extractFileIdFromUrl('https://drive.google.com/uc?export=view&id=FILE123')
    ).toBe('FILE123')

    expect(
      extractFileIdFromUrl('https://drive.google.com/thumbnail?id=FILE456&foo=bar')
    ).toBe('FILE456')
  ...
  it('returns null for unknown patterns', () => {
    expect(extractFileIdFromUrl('https://example.com/not-a-drive-url')).toBeNull()
  })
})
